...
export const App: React.FC = () => {
  ...
  const socketSendCodeUpdate = (data: any) =>
    socket.emit('send-code-update', room, data);
  const socketSendPromptUpdate = (prompt: any) =>
    socket.emit('send-prompt-update', room, prompt);
  const socketToggleTimer = (bool: boolean) =>
    socket.emit('send-timer-update', room, bool);

  useEffect((): any => {
    socket.on('chat-message', (data: any) => dispatch(setGlobalText(data)));
    socket.on('canvas-update', (data: any) =>
      dispatch(setGlobalContextData(data))
    );
    socket.on('code-update', (data: any) => dispatch(setCodeEditorData(data)));
    socket.on('prompt-update', (prompt: any) =>
      dispatch(setPromptData(prompt))
    );
